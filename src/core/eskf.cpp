...
Mat6 Eskf::adaptive_q(Scalar spread, Scalar q_scale, const Scalar* q_floor) {
    // q_pose = q_scale*spread^2 on the diagonal + the additive per-axis floor. `spread` is the
    // weighted-RMS distance of the fused inputs to the TRUE interior robust median (median.cpp) —
    // the correctly-sized per-window uncertainty of the fused delta, so NO /n_eff division (D4:
    // the former approach-A reduction was a fudge for the OLD pinning median's inflated spread;
    // it over-divides the now-correctly-sized spread and makes the filter overconfident). spread
