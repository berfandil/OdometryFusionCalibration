...
- **Correct**: only via optional absolute-reference plugins. **Slice-11 impl**: `Eskf::update(Measurement, χ²)` — innovation `S = H P Hᵀ + R`, NIS `d² = rᵀS⁻¹r` (one LDLT factorization, no explicit inverse), **Mahalanobis-gated** (reject `d² > mahalanobis_chi2`, state untouched), Kalman gain `K = P Hᵀ S⁻¹`, error `dx = K r`, **Joseph-form** covariance for PSD. Injection is in predict's RIGHT-error full-SE(3) tangent: pose `T ← T ∘ exp(dx[0:6])`, twist `xi += dx[6:12]`. Registered `ICorrection`s run in `step()` after predict, before publish (the published frontier + tip reflect the correction). A correction **shrinks P** → mitigates the §10 NEES pessimism *when a ref is present* (the predict-only no-ref path is unchanged).
- **Calibration params are NOT in the state** — they live in the external histograms and feed back via frame-alignment.
- **Optional per-source bias states** (config per-source): when enabled (e.g. a raw-IMU source), absolute-ref updates observe and remove the bias through filter cross-covariance — recovers classic loosely-coupled GPS/INS drift correction. Off by default (core stays bias-free and agnostic). **Slice-11b Option A (implemented)**: when a SINGLE source has `bias_states=true` AND drives the predict alone (1-source / `ColdStart::ReferenceOnly` dead-reckon regime), the ESKF augments to **18-DOF** `[pose; twist; bias(6 body-twist rate)]`. The predict de-biases that source's delta (`Δ_db = Δ∘exp(-b·dt)`) and builds the pose↔bias cross-covariance via `J_pb = -dt·I₆` (the exact leading term); the absolute-ref `update()` then injects `δb` through that cross-covariance and removes the bias (sim: planted bias recovered, drift 16 m → 0.06 m). When the source stops driving alone, a `predict_aug_frozen` path keeps the filter consistent (holds the learned bias, zeros the cross-cov for a clean resume). **Still DEFERRED → 11b residual**: Option B (median-coupled multi-source bias) + the per-`n` χ² Mahalanobis gate. With no `bias_states` source the 12-DOF path is byte-identical.
...
- **Two frontiers**: fusion at `now − delay_fast` (causal, up-to-date) + predict-only tip; calibration at `now − delay_fast − L` (deeper, two-sided-smoothed). **Slice-10 impl**: the deeper frontier is realized by a fixed-capacity ring of depth `L_steps = round(calib_lag_s · tick_rate_hz)`; each step the calibrators consume the lag-`L` entry, with each `per_sensor_kf`-enabled source's twist replaced by the RTS-smoothed (zero-phase) twist and paired with the lag-`L` consensus (time-aligned via per-slot push-seq stamping, so per-source dropout degrades to a clean variance loss, never a bias). Fusion is untouched; with all sources' `per_sensor_kf` off the calibration feed is byte-identical to the causal path.
...
- **Warm-restart persistence** [config on/off, periodic]: persist **calibration state** (committed extrinsics/scale/offsets, reliability weights, lifecycle confidence) → boot near-NOMINAL. **Slice-12 impl**: `serialize`/`deserialize` into a caller fixed buffer (`persistence.hpp` format: magic "OFCP" + `format_version` + `config_hash` + payload + FNV-1a-32 checksum; explicit-LE, padding-free, IEEE-754-bit-exact doubles). The histogram **BINS are NOT persisted** — instead deserialize restores the committed VALUES + per-DOF commit flags and **re-anchors the calibrators** to the restored priors; the Slice-8 hysteresis (`commit_gate_reanchor`, plus an `offset_restored` latch for the time-offset DOF) holds the restored-committed DOFs committed while the histograms re-fill, so fusion uses the restored priors immediately = near-NOMINAL (the in-progress vote distribution is the only thing lost, and it re-concentrates). Restore also validates each extrinsic R is orthonormal (`CorruptData` otherwise). **Double-buffer ping-pong** (files A/B + sequence counter, each version-tagged + checksummed; write inactive → fsync → flip) so a crash mid-write never corrupts the last good state. **Invalidate** on setup change: manual delete + **auto config-hash guard** (reject persisted state whose config/prior signature mismatches the current rig). Core does `serialize/deserialize` into fixed buffers; file IO lives in an adapter.
...
- **Source plugin**: one query `delta(t0,t1) → (SE3 motion, Σ6×6)`. An adapter converts the native form (twist → integrate, increments → compose, absolute → difference) and interpolates in-window. **Confidence** = Σ: provider's native Σ **⊕** a modeled noise Σ (combine config — default **sum**; native missing → identity).
  - **INPUT CONTRACT — feed RAW odometry, do NOT pre-(Kalman-)filter it.** Each source must deliver its raw per-window relative motion + native Σ; do not run a Kalman/RTS/low-pass filter on a source before adding it to this system. **Why**: (1) a filter correlates noise across windows, so the increments are no longer independent — which breaks the ESKF's per-window-independent process-noise (`Q`) model and the spread→`Q` adaptive sizing; (2) a fixed-lag filter adds latency, pushing the causal fusion frontier back; (3) smoothing shrinks the inter-source spread → shrinks adaptive `Q` → makes the published covariance over-confident; (4) it can mask the systematic bias the online calibrator + bias-state path are there to observe and remove. This system does its own per-sensor smoothing internally **on the deeper calibration frontier only** (Slice 10 `TwistSmoother`) — fusion is deliberately driven by raw, causal deltas. If a source can only emit a pre-filtered stream, treat its reported Σ as already-inflated and expect degraded covariance consistency.
- **Absolute-ref plugin** (optional, 0..N): `evaluate(state) → (residual = z ⊟ h(x), H, R)` + timestamp. Standard ESKF update at the frontier, **Mahalanobis-gated**. Covers position / pose / orientation agnostically. **Loop-closure excluded** (needs retained past states ⇒ smoother territory). Concrete sensors (GPS, map-match) ship as adapters — the first concrete one is `ofc_adapters::GpsCorrection` (Slice 11b, `4bce8d1`): geodetic GPS → WGS-84 ECEF → ENU about a datum → odom via a configurable `odom_from_enu` SE3 → dim=3 position fix with a lever arm (`H=[R|−R·[l]×|0]`).
- **Output**: transport-agnostic, **callback + poll**. Per-tick result struct: frontier state (pose-in-odom + twist + Σ), predicted tip (+ inflated Σ), per-sensor calibration snapshot (extrinsic, scale, time-offset, and **per-DOF confidences**: `confidence`=time-offset, `extrinsic_confidence`=yaw/pitch so(3), `scale_confidence`=scale — each DOF converges in its own regime), health/diagnostics (per-source residuals, weights, per-source `reliability` + mean-residual `bias` (Slice 9), gating regime, histogram confidences, lifecycle phase, absolute-ref correction diagnostics (evaluated/applied/rejected counts + last NIS, Slice 11), and a coarse `readiness` scalar in [0,1] — Slice 3: Init 0.0 / Warmup 0.25 / Degraded 0.6 / Nominal 1.0). Pose lives in an **odom frame anchored at init** (drifts — inherent to odometry-only). Extrinsics reported in the base frame. ROS/DDS/zmq/logging are adapters outside core.
...
- **NEES / NIS consistency** (Monte-Carlo): a filter that publishes Σ must prove it is neither over- nor under-confident. **Slice-14 status**: NEES Monte-Carlo harness implemented (6-DOF pose NEES from the published Σ vs sim GT, ensemble over seeds). **Finding (init-P pessimism — RESOLVED)**: the published Σ was originally **grossly pessimistic** — ensemble-mean NEES ≈ 0.13 vs DOF 6 (~46×) because the ESKF initialized `P = I₁₂` (≈100× the steady-state error). **Fixed**: the filter now initializes `P = 0` at the gauge-anchored first fuse and lets the first `predict()` establish the one-window covariance `blkdiag(q_pose, q_pose/dt²)` (no magic seed, no double-count). Ensemble NEES improved to **≈ 0.35** (~17×). **Residual — approach A PARTIAL FIX (`730bcfa`)**: a read-only spike attributed the residual to the right-error `Ad(delta⁻¹)` **translation-block inflation** AMPLIFYING a per-window Q that over-states the FUSED median's accuracy ~14–17× (the published P translation diagonal climbs to **tens of m²** — ~77 m² by ~32 m of travel, not the "~2.5–5 m²" earlier comments said — while realized translation error² grows ~6× over the span P grows ~23×; the rotation block stays ~0.14 rad² with no distance coupling). Q-magnitude tuning can't touch the growth *shape* (driving Q→1e-12 leaves the 23× factor unchanged — they're additive offsets). **Approach A** (`adaptive_q_source_reduction`, default on) divides the spread-derived Q by the participating-median source count `n_eff` (the geometric median of `n` agreeing sources has ~`1/n` the single-source per-window variance) — a principled, parameter-free, **safe** (never overconfident) reduction: ensemble NEES **0.35 → ~1.0** (~17× → ~6× pessimistic). The residual ~6× was the un-calibrated `q_scale=1` coefficient — **now calibrated** (Slice-14 sweep, `983fa65`): `q_scale 1.0 → 0.5`, chosen SAFETY-FIRST across {nees_traj, mixed, turning, straight} × {1×,2× noise} as the max pessimism cut subject to NEVER overconfident — no-ref pose NEES rose **~1.0 → ~2.07** (nees_traj; worst-case ~2.9 at 1× / ~3.5 at 2×). NOT pushed to NEES≈6 (≈`q_scale 0.17`): that overfits one trajectory and the sim under-states real model mismatch, so the gap to 6 is a **deliberate safety margin** (mild pessimism preferred over any overconfidence). Closing the rest cleanly would need the `Ad(δ⁻¹)` distance-*shape* model and/or an NHC no-ref correction (approach B). The NEES test pins the calibrated value (band [1.50,2.70]) + a knob-OFF guard ([0.45,1.00], the /n_eff reduction independent of q_scale) + a separate multi-trajectory `test_cov_calibration` guard (never-overconfident across the set) + `CHECK_FALSE(truly_consistent)` so a regression re-trips. **NIS** is computable (Slice 11 `ICorrection` update path): the NIS Monte-Carlo on accepted absolute-ref fixes is in place — ensemble-mean ≈ 2.4 vs measurement DOF 3 (mildly conservative, as P re-inflates between fixes; the correction shrinks P toward consistency). The golden-regression net (deterministic recorded-scenario replay + committed numeric snapshot) is in place.
