...
- **Real-data ingestion + replay** (relaxed-edge adapters, `ddcf436`): `ofc_adapters::CsvSource` (an `ISource` reading a generic CSV — absolute/increment/twist forms, hand-rolled dep-free parser, backed by `SourceBuffer`), `CsvGtTrack` (interpolated GT lookup), and `ReplayHarness` — the **real-data analogue of the sim `Rig`**: it pumps `Estimator::step()` over a tick timeline and computes drift + 6-DOF NEES (the `test_validation` `pose_nees` convention) + accepted-GPS-fix NIS vs a GT track. Driven by the `ofc_replay` CLI on a manifest (an extended `ConfigLoader` INI). This is how a real dataset (KITTI/KAIST/NCLT/EuRoC, converted to the CSV schema once) is run + scored against the sim-calibrated covariance. Schema + manifest in `adapters/README.md`.
...
- **NEES / NIS consistency** (Monte-Carlo): a filter that publishes Σ must prove it is neither over- nor under-confident. **Current status (`1142e41`): NEAR-CONSISTENT, never overconfident.** Ensemble-mean 6-DOF pose NEES ≈ **4.82** vs DOF 6 (nees_traj; worst-case across {nees_traj, mixed, turning, straight} × {1×,2× noise} ≈ 4.85, every trajectory < 6) — approaching consistency from below, the gap a deliberate safety margin (the sim under-states real model mismatch). Pinned by the NEES test band [4.0,5.6] + the multi-trajectory `test_cov_calibration` never-overconfident guard (<5.5) + `CHECK_FALSE(truly_consistent)` (4.82 < the chi² interval [5.94,6.06]). **History (how we got here)**: (1) the ESKF originally seeded `P = I₁₂` → grossly pessimistic NEES ≈ 0.13 (~46×); the **init-P fix** (`70c7d38`: seed P=0 at the gauge-anchored first fuse, first `predict()` establishes `blkdiag(q_pose, q_pose/dt²)`) raised it to ≈ 0.35. (2) Steps 3–4 (`730bcfa` `/n_eff` + `983fa65` `q_scale=0.5`) drove NEES toward ~2.07 — but those were **calibrating around the D3 median-pinning bug** (the solver returned ONE source verbatim, so it averaged nothing and the "median variance reduction" rationale for `/n_eff` was false). (3) Fixing the median to a true interior blend (D3) made the spread correctly sized; `/n_eff` then OVER-divided Q → overconfident (NEES ~20), so it was **removed**, and `q_scale` recalibrated to **0.7** against the true median → the near-consistent 4.82 above. With the true median, smaller `q_scale` → larger NEES (opposite of the pinning-median sign). Strict per-trajectory consistency to exactly DOF=6 would need an `Ad(δ⁻¹)` distance-*shape* covariance model and/or an NHC no-ref correction (deferred). **NIS** (Slice 11 `ICorrection` update path): ensemble-mean ≈ **2.7** vs measurement DOF 3 (mildly conservative, as P re-inflates between fixes). The golden-regression net (deterministic replay + committed numeric snapshot) is in place.
