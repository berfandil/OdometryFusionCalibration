...
**Follow-up — BUILT as Slice 19d (`4a77219`)**: warm-start the boost from `rot_weight_prior` (prior as the abstain-window default, monitor refines once scored) erases the latency cost while keeping auto-discovery. **Design (Option C — the monitor ABSORBS the prior when ON, no new knob, no double-count)**: `HeadingMonitor::boost()` returns `clamp(rot_weight_prior, 1, boost_max)` during abstain (was 1.0), the computed boost once scored; the weight assembly drops the separate `·rot_weight_prior` when the monitor is ON (`weights_rot = wr·hm_boost`; OFF keeps `wr·rot_weight_prior`, byte-identical). `rot_weight_prior=1` default → abstain boost 1.0 → the 19c abstain pin still holds. Fixes a latent DOUBLE-COUNT (monitor ON + prior=10 + scored gave `·10·10`). **Validated (urban12, monitor + `rot_weight_prior=10` + cov_floor=300)**: local rot p50 **0.23°** = identical to the manual-prior-only result, vs the monitor-only **0.49°** latency cost — the FOG warm-starts to boost 10 from t=0 AND the monitor independently scores it 10 (seamless handoff, no chatter). Unit 337/78351 + adapters 46/772 green. 3 new tests (warm-start, no-double-count, transition) + the abstain pin's boost-diagnostic updated (fusion still byte-identical at prior=1).
