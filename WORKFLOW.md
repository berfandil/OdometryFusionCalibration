...
- [x] **Median-pinning fix + covariance recalibration** (`1142e41`, largest change): the n>=3 geometric-median solver was PINNING (init at the highest-weight vertex -> d=0 self-weight ~1e9 -> 1-iter pin -> returned that source verbatim, no blending, a high-weight outlier returned with ZERO rejection). Fixed with off-vertex weighted-mean init + Vardi-Zhang coincident-vertex guard -> true interior robust median (high-weight-outlier guard added; the old outlier tests passed by accident). This SUPERSEDED steps 3-4 (which were calibrating around the bug): approach A `/n_eff` REMOVED (its "median variance reduction" rationale was false for the pinning median; it over-divided Q with the true median -> overconfident), `q_scale` recalibrated 0.5 -> 0.7 against the true median. Result: NEES 2.07 (pessimistic-by-design) -> ~4.82 (NEAR-CONSISTENT, never overconfident). Found + de-risked by 5 spikes; exposed 2 latent calibration bugs (logged in ISSUES). Independent review: no test weakened to mask a regression (green: unit 211/8995 + adapters 27/372).
- [x] **Calibration bug 1(a) — scale_hist boundary commit FIXED** (`3bd91e2`): the median fix exposed a pre-existing bug — `scale_hist` inherited the generic `[-1,1]` HistogramConfig default, so a unit scale ratio (1.0) hit the half-open upper boundary → last-bin clamp → `mode()` returned the boundary-bin center 0.984375 → a true unit scale committed as 0.984. Fix = default `scale_hist` to `[0.5,1.5]` (1.0 interior) + a `validate()` guard; re-enabled `scale_calib` in test_sim's `make_rig_config` (dropped the workaround); new regression test. The noise-free golden `c1->scale` corrected 1.08281→1.1 (1.08281 = 1.1×0.984375, independent confirmation). Diagnosed by a read-only spike; bug 1(b) (partial scale recovery 1.07 vs 1.2) was found NOT a bug (sparse-vote/noise artifact, vote is vs the reference not the median) → documented known-limitation, no change. Reviewed clean (green: unit 212/9301 + adapters 27/372).
- [x] **Real-data CSV ingestion + replay harness** (`ddcf436`, relaxed-edge scaffolding): `CsvSource`(ISource, 3 forms, dep-free parser, SourceBuffer-backed) + `CsvGtTrack` + `ReplayHarness` (drift/NEES/NIS vs GT — the real-data analogue of the sim Rig) + the `ofc_replay` CLI (manifest = extended config-loader INI). Equivalence-tested CSV==in-memory to 1e-6. Unblocks real-dataset testing (KITTI/KAIST/NCLT/EuRoC → CSV via a one-time conversion script); schema in `adapters/README.md`. Reviewed clean (green: unit 212/9301 + adapters 36/635).
