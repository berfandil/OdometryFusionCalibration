...
- **Remaining**: the non-covariance placeholders (`excitation_min_var`, `kf_process_noise`, `match_metric`, straight/turn calibration gates — separate pass, observability self-tests pin them functionally); and the `Ad` distance *shape* model and/or an NHC no-ref correction for strict per-trajectory consistency to exactly DOF=6 — **now real-data-motivated**: the KITTI run (28 drives, single-source) showed the predict-only covariance is badly overconfident (NEES up to ~1e5 vs DOF 6) and a single `q_floor` can't fix it across drives (error growth is trajectory-dependent) — the shape model is what's actually needed. See `q_scale`/`q_floor` enabler (`e03b237`) + memory `real-dataset-testing`.
