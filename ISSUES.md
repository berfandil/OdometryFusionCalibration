...
## Slice 15b — Bounded heading injection from position-only corrections (lever C)  `[ ]` DESIGN
**Goal**: stop a position fix from over-rotating heading via the `P` trans-rot cross-cov when its residual is large (the pinned urban12 t=1929 s 68° kick). **Approach** (full design `SLICE15B_BOUNDED_HEADING_INJECTION.md`): **C4 — residual-gated rotation-row suppression**. For a rotation-unobserving correction (`H` rotation cols ≈ 0) with large per-DOF residual `dbar > rot_suppress_kappa`, attenuate ONLY `K`'s rotation rows by `rot_suppress_kappa/dbar` (translation pull untouched, Joseph stays consistent). `correction_rot_suppress_kappa=0` default → opt-in/inert. Orthogonal to Slice-15 Huber (A scales whole gain on huge outliers; C4 protects heading on in-gate-but-large position residuals). **Done when**: t=1929 kick < ~10° with translation still pulled; urban12 tail ≪ 3 km / `local max` ↓; urban07/17 `local p50` ±10%; sim bit-identical at kappa=0. **Out of scope**: the upstream 522 s heading-drift (DR/FOG-IMU heading-hold quality, separate). **Deps**: Slices 13, 15.
