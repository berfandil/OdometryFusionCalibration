...
- **Remaining**: the non-covariance placeholders (`excitation_min_var`, `kf_process_noise`, `match_metric`, straight/turn calibration gates — separate pass, observability self-tests pin them functionally); and the `Ad` distance *shape* model and/or an NHC no-ref correction for strict per-trajectory consistency to exactly DOF=6.
- **Latent bugs surfaced by the median fix (newly EXPOSED, were MASKED by the pinning bug — own follow-ups)**: (1) **cold-start scale-calib spurious commit** — when a source's `prior_scale` already equals truth, the Phase-1 scale calibrator can still commit a wrong scale (~0.984, a 63/64 coarse-histogram/bootstrap artifact); the old pinning median hid it by returning the clean reference, the true median blends it in (`test_sim.cpp` works around it with `scale_calib=false` since those are fusion-tracking tests). (2) **calibrator consensus contamination** — a biased source's scale recovers only partially (1.07 vs planted 1.2) because the interior median blends in the source being calibrated; candidate fix = exclude the source-under-calibration from its own consensus, or use `ReferenceOnly` cold-start. Neither blocks the median fix.
