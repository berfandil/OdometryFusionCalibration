...
**urban12 reframed (next)**: even with healthy gate + realistic R, p50=0.17 m but one window still hits 476 m and corrupts the global frame → a **localize-the-bad-window** problem (which timestamp/event: GPS-denied stretch → DR drift → returning in-gate fix yank?), NOT a robust-update gain problem. Needs targeted diagnosis, not another gain knob (lever #4 that helped-but-didn't-fix).
