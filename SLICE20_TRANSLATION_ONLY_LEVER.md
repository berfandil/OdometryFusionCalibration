...
**Status (20b implemented, TDD, gate green)**: per-axis accessors (`translation_confidence_axis`/`xyz_axis_mass`), estimator `lever_committed_xyz[cap][3]` + three independent per-axis `commit_gate_reanchor` gates (translation_only only; non-flagged stay whole-lever, byte-identical with the per-axis array mirroring the whole flag), per-axis publish into `prior_extrinsic.t`, `CalibSnapshot.translation_committed_xyz[3]` diagnostics, persistence **format v4** (3 per-axis flags appended after scale2; whole bool re-derived as the OR on restore; pre-v4 cold-start). Tests 9-11 added to `test_calib_translation_only.cpp` (per-axis headline cx 0.61 commits / cy 0.17 withheld / cz empty while the whole-lever min 0.17 would NOT commit; non-flagged per-axis-mirror invariant pinned every step; v4 round-trip incl. a blob-parsed per-axis fold + a v3-stamp reject), the existing 8 + the rot3d/joint-scale persistence version pins updated to v4 (a version-constant pin + the rot3d blob's manual schema parse, NOT a behavioral weakening). Full gate green (unit 334 cases / 70252 assertions + adapters 46/772). Real-data (item 12) is the orchestrator's post-merge step.
...
2. **Per-axis lever commit** — DONE (Slice 20b, `c85442e`, see §4b). Each lever axis commits independently; sim-validated (a clean axis commits where the whole-lever min could not — `cx=0.61` commits, `cy=0.17`/`cz=0` held). **Result (nuScenes radar, per-axis re-validation)**: `lever_committed_xyz = [0,0,0]` on ALL 5 radars — STILL no commit. Every per-axis concentration sits ~0.55 (src2's observed axes are all in [0.553, 0.6)), just below the 0.6 gate. The per-axis mechanism is correct + necessary, but on this real radar data NO single axis is clean enough to clear the gate.
3. **Lower per-window radar noise = THE REMAINING WALL (now isolated)**. With the footgun fixed, z-min unpinned, and per-axis commit in place, the ONLY thing keeping the real radar lever uncommitted is the raw per-window Doppler-ego-velocity noise holding every axis's concentration at ~0.55 < 0.6. The readout overshoots truth (~20-28 cm), so LOWERING the gate would commit a wrong-ish lever (violates never-commit-garbage) — NOT the fix. The real unlock is better radar odometry: tighter Doppler RANSAC / scale handling, longer accumulation windows, or a batch lever-only solve at the deeper calibration frontier instead of per-window histogram votes. The along-track axis stays a sensor SNR wall regardless (prototype-confirmed). **The calibrator machinery is complete and correct; committing a real radar lever is now a radar-front-end (odometry quality) problem, not a calibration one.**
