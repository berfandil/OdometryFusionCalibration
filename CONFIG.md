...
| `q_scale` | double | 1.0 | (0, ∞) | Multiplier on the spread-derived Q. **Tuned placeholder** — the spike behind Slice-14 approach A found `spread²` (inter-source disagreement) over-states even a SINGLE source's per-window error by ~6× at `q_scale=1`, the residual pessimism after the median-variance reduction below. Trajectory-dependent; calibrating it is the open CONFIG-sweep item. |
