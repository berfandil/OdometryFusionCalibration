...
| `cov_floor_m2` | double | 25 | ≥0 | GPS ENU measurement-covariance FLOOR (m²) for the `GpsCorrection` adapter (per-fix `cov_enu` floored by this). **R-side NEES sweep (2026-06-15, `GPS_R_NEES_SWEEP.md`)**: 25 is **17–300× too small** — with-GPS pose NEES is 103/1889/1198 on urban07/12/17 (vs DOF 6). Raising it monotonically reduces NEES AND (urban07) IMPROVES local drift (p50 0.98→0.38 m at cov 25→800: a too-small R over-trusted noisy GPS and injected jitter); urban12 worst-window 211→108 m; tails flat. **Recommended ~200–400** (strictly better — accuracy + consistency, config-only). Full NEES~6 consistency is NOT a scalar-R problem: the honest-R differs ~7× cross-drive (urban07 ~1500, urban12/17 ~10000) and is implausibly large for VRS-RTK (σ~100 m) → it is dominated by GPS-vs-GT FRAME disagreement (VRS-RTK vs SLAM-GT differ ~8–26 m), not measurement noise; the KAIST per-fix variance is a flat 0.25 m² nominal (no multipath signal). A per-fix/multipath-aware R MODEL + GPS-vs-GT frame-offset characterization is the principled fix (ISSUES; parallel to the `Ad`-shape `q_floor`). **Follow-up DONE (2026-06-21, `GPS_R_NEES_SWEEP.md` §5)**: the innovation analysis (`tools/gps_innovation_analysis.py`) showed the GPS-vs-FUSED bias is small (0.3–0.5 m) on clean drives — the huge NEES is the GPS/fused-vs-GT FRAME offset, not measurement R; the honest measurement R is small + cross-drive-varying (2→22 m²); urban12 is an OUTLIER drive. The `adaptive_r` model below makes the NIS consistent but PROVES the NEES is a GT-frame bias (it explodes under honest small R). `cov_floor_m2` stays the recommended accuracy knob (the band-aid); `adaptive_r` is the honest measurement model. |
| `adaptive_r` (+ `adaptive_window`/`adaptive_min_samples`/`adaptive_r_floor_m2`) | bool / int,int,double | false / 60,10,0.25 | — | **`[gps]` manifest keys; `GpsConfig` adapter, opt-in.** Innovation-adaptive ROBUST GPS R (`GPS_R_NEES_SWEEP.md` §5): when on, `evaluate()` sets `R_odom = diag(max((1.4826·MAD_i)², floor))` from the robust scale of the last `adaptive_window` innovations per odom axis (falls back to `cov_enu+cov_floor_m2` until `adaptive_min_samples` seen). MAD-robust → tracks the BULK measurement spread, immune to gross multipath fixes (which then exceed the gate and get rejected) → a SELF-CALIBRATING cross-drive-consistent measurement R where a scalar `cov_floor` cannot be. **Default OFF = byte-identical to the `cov_enu+cov_floor` path.** In-loop (KAIST): makes the un-gated NIS consistent (0.06/0.23/0.03 → 1.36/0.68/1.13 across urban07/12/17) + improves the multipath drives' p50 drift, BUT NEES-vs-GT explodes (honest small R confidently tracks the GT-offset GPS) — confirming the residual error is a GT-frame BIAS, not R. Opt-in for outlier-prone GPS / measurement-consistency; NOT a drop-in accuracy win. |
