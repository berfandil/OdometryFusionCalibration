...
- **Gate green: ctest 2/2 — `unit` 212 cases / 9301 assertions + `adapters` 27 cases / 372 assertions.** Pushed through `1996519`; locally ahead by the **median-pinning fix** (`1142e41` — the largest change; fixes a core-fusion bug, removes the now-superseded `/n_eff`, recalibrates `q_scale` 0.5→0.7), its doc-sync (`1996519`, pushed), and the **`scale_hist` boundary-bug fix** (`3bd91e2` — bug 1(a): scale_hist default `[-1,1]`→`[0.5,1.5]` so a unit scale ratio isn't clamped to the boundary bin 0.984; re-enabled test_sim `scale_calib`; +`validate()` guard) — `3bd91e2` + this doc commit are unpushed. (The gate also builds the relaxed-edge adapters — `dev.ps1` sets `OFC_BUILD_ADAPTERS=ON`, so CTest runs both `unit` and `adapters`. **Use `dev.ps1 -Task clean` before A/B covariance/median measurements — stale ninja artifacts gave misleading NEES numbers during this work.**)
- **Done (all green):**
