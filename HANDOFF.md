...
- **Gate green: ctest 2/2 — `unit` 208 cases / 9154 assertions + `adapters` 27 cases / 372 assertions.** ~70 commits on `main`. Feature work through **11b Option A** + the **per-`n` χ² gate** (`e8491dd`) + the **GPS adapter** (`4bce8d1`) + the **median-variance Q reduction** (`730bcfa`) + the **`q_scale` calibration sweep** (`983fa65`, Slice-14) is committed locally; those commits + the doc-refresh commits are ahead of the remote (`0c19628`) until pushed. (The gate also builds the relaxed-edge adapters — `dev.ps1` sets `OFC_BUILD_ADAPTERS=ON`, so CTest runs both `unit` and `adapters`. `unit` is ~17 s now — the added multi-trajectory NEES Monte-Carlo guard.)
- **Done (all green):**
...
- **Covariance pessimism — init-P + median-variance reduction (A) + `q_scale` calibration DONE; residual is the deliberate safety margin + `Ad` shape**: the Monte-Carlo NEES (6-DOF pose, full-SE(3) `se3::log` tangent) was ensemble-mean ≈ **0.13** vs DOF **6** (~46×) because the ESKF initialized `P = I₁₂`. **Init-P fix** (`70c7d38`): init `P = 0` at the gauge-anchored first fuse, first `predict()` establishes `blkdiag(q_pose, q_pose/dt²)` → NEES ≈ **0.35** (~17×). **Approach A** (`730bcfa`): a read-only spike attributed the residual to the spread-derived Q over-stating the FUSED median's accuracy ~17× (`spread` = inter-source disagreement; the median of `n` agreeing sources has ~`1/n` the variance), amplified by the `Ad(δ⁻¹)` translation transport (P_tt to ~tens of m² — ~77 m² by ~32 m — while realized error² grows ~6× over the span P grows ~23×; rotation block fine ~0.14 rad²). Fix = `adaptive_q_source_reduction` (default on) divides the spread term by `n_eff` → NEES ≈ **1.0**, principled + **safe**. **`q_scale` calibration** (`983fa65`): swept across {nees_traj, mixed, turning, straight} × {1×,2× noise}, `q_scale 1.0 → 0.5` chosen SAFETY-FIRST (max pessimism cut s.t. never overconfident) → NEES ≈ **2.07** (worst-case ~2.9/~3.5). NOT pushed to ≈6 (≈`q_scale 0.17` overfits + risks overconfidence; sim under-states real model mismatch) — the gap to 6 is a **deliberate safety margin**. The reduction is *multiplicative* (q_scale), NOT an `Ad`-shape fix; full strict per-trajectory consistency would need the distance-*shape* model and/or an NHC no-ref correction (approach B). Slice 11 mitigates it WHEN an absolute ref is present (correction shrinks P → NIS ≈ 2.4→2.8 vs DOF 3). Guards: NEES band [1.50,2.70] + knob-OFF guard [0.45,1.00] (proves /n_eff independent of q_scale) + `test_cov_calibration` (never-overconfident across the trajectory set) + `CHECK_FALSE(truly_consistent)` — any regression (revert `I₁₂`, break the reduction, or over-lower q_scale) re-trips.
...
- Not yet built: 11b residual (Option B only), ROS node + durable fsync (13b). Partial: validation (14) — NEES/NIS/golden + init-P fix (`70c7d38`) + median-variance Q reduction (`730bcfa`, A) + `q_scale` calibration (`983fa65`) DONE; remaining = the non-covariance placeholders (excitation_min_var, kf_process_noise, match_metric, calib gates) + (for strict NEES consistency) the `Ad` distance-shape model / NHC no-ref correction (B).

---

## 7. Resume in one line

All numbered roadmap slices (0–14) are addressed; init-P fix + 11b Option A + the per-`n` χ² gate + the GPS adapter + the median-variance Q reduction + the `q_scale` calibration (Slice-14 A + sweep) have landed. Remaining work is carve-outs + polish:
- **11b residual** — Option A DONE + per-`n` χ² gate DONE (`e8491dd`) + GPS adapter DONE (`4bce8d1`). Deferred: **Option B** only (median-coupled multi-source bias — design in `ISSUES.md` Slice-11b DESIGN NOTE).
- **13b** (real ROS node + bag round-trip + durable `fsync`) — env-blocked (no ROS on the dev box).
- **Slice-14 remaining placeholders** — the NON-covariance knobs (`excitation_min_var`, `kf_process_noise`, `match_metric`, straight/turn calibration gates); the load-bearing `q_scale` is done (`983fa65`, 1.0→0.5 safety-first).
- **`Ad` distance-shape covariance model and/or an NHC no-ref correction (approach B)** for strict per-trajectory no-ref NEES consistency (A + the `q_scale` calibration already cut the pessimism ~17×→~3×; the rest is the deliberate safety margin / the `Ad` shape).
