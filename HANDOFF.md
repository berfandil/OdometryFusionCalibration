...
- **Gate green: ctest 2/2 — `unit` 195 cases / 8274 assertions + `adapters` 20 cases / 319 assertions.** 53 commits on `main`, working tree clean. Remote synced through Slice 12; the Slice-13 commits are local (not pushed). (The gate now builds the relaxed-edge adapters too — `dev.ps1` configures `OFC_BUILD_ADAPTERS=ON`.)
- **Done (all green):**
...
- Not yet built: per-source bias states + GPS adapter (11b), ROS node + durable fsync (13b). Partial: validation (14) — NEES/NIS/golden done, CONFIG "tuned"-placeholder sweep + the init-P covariance fix pending.

---

## 7. Resume in one line

All numbered roadmap slices (0–14) are addressed. Remaining work is the carve-outs + polish: **11b** (per-source bias states + GPS adapter), **13b** (real ROS node + durable fsync), the **init-P covariance fix** (seed `P` ≪ `I₁₂` → strictly-consistent NEES), and the **Slice-14 CONFIG "tuned"-placeholder sweep**. Pick one, author the brief from `WORKFLOW.md`'s template, run the cycle in §4. Verify with §2. Done.
