...
- **Gate green: ctest 2/2 — `unit` 206 cases / 8352 assertions + `adapters` 27 cases / 372 assertions.** ~65 commits on `main`. Feature work through **11b Option A** + the **per-`n` χ² gate** (`e8491dd`) + the **GPS adapter** (`4bce8d1`) is committed locally; those commits + the doc-refresh commits are ahead of the remote (`0c19628`) until pushed. (The gate also builds the relaxed-edge adapters — `dev.ps1` sets `OFC_BUILD_ADAPTERS=ON`, so CTest runs both `unit` and `adapters`.)
- **Done (all green):**
...
- Not yet built: 11b residual (Option B only), ROS node + durable fsync (13b). Partial: validation (14) — NEES/NIS/golden + init-P fix DONE (`70c7d38`); remaining = the CONFIG "tuned"-placeholder sweep + (for strict NEES consistency) a distance-aware covariance model / no-ref correction.

---

## 7. Resume in one line

All numbered roadmap slices (0–14) are addressed; init-P fix + 11b Option A + the per-`n` χ² gate + the GPS adapter have landed. Remaining work is carve-outs + polish:
