...
- **Gate green: ctest 2/2 — `unit` 211 cases / 8995 assertions + `adapters` 27 cases / 372 assertions.** ~72 commits on `main`. Feature work through **11b Option A** + the **per-`n` χ² gate** (`e8491dd`) + the **GPS adapter** (`4bce8d1`) + the Slice-14 covariance work + the **median-pinning fix + covariance recalibration** (`1142e41` — the largest change; fixes a core-fusion bug, removes the now-superseded `/n_eff`, recalibrates `q_scale` 0.5→0.7) is committed locally; ahead of the remote (`0c19628`) until pushed. (The gate also builds the relaxed-edge adapters — `dev.ps1` sets `OFC_BUILD_ADAPTERS=ON`, so CTest runs both `unit` and `adapters`. **Use `dev.ps1 -Task clean` before A/B covariance/median measurements — stale ninja artifacts gave misleading NEES numbers during this work.**)
- **Done (all green):**
...
- Not yet built: 11b Option B (now RECONSIDERABLE — the median fix makes the per-source `ωᵢ` real; see ISSUES Slice-11b UPDATE), ROS node + durable fsync (13b). Partial: validation (14) — NEES/NIS/golden + init-P fix (`70c7d38`) + the **median-pinning fix + `q_scale=0.7` recalibration** (`1142e41`) DONE → NEES near-consistent ~4.82; `/n_eff` (A) was REMOVED. Remaining = the non-covariance placeholders (excitation_min_var, kf_process_noise, match_metric, calib gates); the `Ad` distance-shape model / NHC no-ref correction for strict DOF=6; and the two **latent calibration bugs the median fix exposed** (cold-start scale spurious commit; calibrator consensus contamination — ISSUES Slice 14).
...
- **Two latent calibration bugs the median fix EXPOSED** (ISSUES Slice 14): cold-start scale-calib spurious commit; calibrator consensus contamination (candidate fix: exclude the source-under-calibration from its consensus).
