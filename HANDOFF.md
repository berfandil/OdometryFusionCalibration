...
- **Gate green: ctest 2/2 — `unit` 207 cases / 8537 assertions + `adapters` 27 cases / 372 assertions.** ~68 commits on `main`. Feature work through **11b Option A** + the **per-`n` χ² gate** (`e8491dd`) + the **GPS adapter** (`4bce8d1`) + the **median-variance Q reduction** (`730bcfa`, Slice-14 approach A) is committed locally; those commits + the doc-refresh commits are ahead of the remote (`0c19628`) until pushed. (The gate also builds the relaxed-edge adapters — `dev.ps1` sets `OFC_BUILD_ADAPTERS=ON`, so CTest runs both `unit` and `adapters`.)
- **Done (all green):**
...
- **Covariance pessimism — init-P RESOLVED + median-variance reduction (A) DONE; residual is the `q_scale` coefficient + `Ad` shape**: the Monte-Carlo NEES (6-DOF pose, full-SE(3) `se3::log` tangent) was ensemble-mean ≈ **0.13** vs DOF **6** (~46×) because the ESKF initialized `P = I₁₂`. **Init-P fix** (`70c7d38`): init `P = 0` at the gauge-anchored first fuse, first `predict()` establishes `blkdiag(q_pose, q_pose/dt²)` → NEES ≈ **0.35** (~17×). **Approach A** (`730bcfa`): a read-only spike attributed the residual to the spread-derived Q over-stating the FUSED median's accuracy ~17× (`spread` = inter-source disagreement; the median of `n` agreeing sources has ~`1/n` the variance), amplified by the `Ad(δ⁻¹)` translation transport (which inflates P_tt to ~tens of m² — ~77 m² by ~32 m — while realized error² grows ~6× over the span P grows ~23×; rotation block fine ~0.14 rad²). Fix = `adaptive_q_source_reduction` (default on) divides the spread term by the participating-median source count `n_eff` → NEES ≈ **1.0** (~6×), principled + **safe** (never overconfident). NOT `q_scale`/`q_floor`-tunable as an *offset* (shape is `Ad`-driven); the reduction is *multiplicative*. The **remaining ~6×** is the un-calibrated `q_scale=1` coefficient (CONFIG sweep, trajectory-dependent) + the `Ad` distance *shape*; full strict consistency would also want a distance-shape model and/or an NHC no-ref correction (approach B). Slice 11 mitigates it WHEN an absolute ref is present (correction shrinks P → NIS ≈ 2.4→2.7 vs DOF 3). The NEES test pins the post-A value (band [0.70,1.50]) + a knob-OFF guard reproducing ~0.35 + `CHECK_FALSE(truly_consistent)` so a regression (e.g. reverting to `I₁₂`, or the reduction breaking) re-trips.
...
- Not yet built: 11b residual (Option B only), ROS node + durable fsync (13b). Partial: validation (14) — NEES/NIS/golden + init-P fix (`70c7d38`) + median-variance Q reduction (`730bcfa`, A) DONE; remaining = the CONFIG `q_scale`-coefficient sweep + (for strict NEES consistency) the `Ad` distance-shape model / NHC no-ref correction (B).

---

## 7. Resume in one line

All numbered roadmap slices (0–14) are addressed; init-P fix + 11b Option A + the per-`n` χ² gate + the GPS adapter + the median-variance Q reduction (Slice-14 A) have landed. Remaining work is carve-outs + polish:
...
- **Slice-14 CONFIG "tuned"-placeholder sweep** — chiefly the `q_scale` coefficient (the residual ~6× of the no-ref NEES pessimism after approach A's median-variance reduction; `730bcfa`).
- **`Ad` distance-shape covariance model and/or an NHC no-ref correction (approach B)** for strict no-ref NEES consistency (the median-variance reduction (A) already cut it ~17×→~6×).
