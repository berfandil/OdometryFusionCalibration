...
**REAL 4D RADAR — RADIal: DONE (2026-06-19) — NEGATIVE RESULT, the synth-4D rotation unlock does NOT transfer to real imaging radar. Full write-up: `RADAR_SCAN_ODOMETRY.md` §5.** Recon corrected the HANDOFF's assumptions: RADIal `Ready_to_use` is NOT extracted (a 12x10 GB split zip, parts MISNAMED `RADIal-XXX.zYY` so 7z/unzip can't read them — wrote a Python multipart-zip reader + run-extractor under `C:\workspace\data\RaDIaL\_recon\`), ships radar_PCL (sparse 3D+Doppler point cloud, ~500 pts/frame — NO CFAR step needed, the detection is already done) + a dense Scala lidar, and has **NO GPS/CAN/ego-pose** (those live only in the raw sequences we don't have — my earlier "GPS->GT, CAN->odom" assumption was wrong). So lidar ICP odometry (`tools/proto_radial_lidar_odom.py`) became the reference + GT, radar (`tools/radial_to_csv.py`, the proven 3D scan-match) the source under calibration. On the best run (an 81 deg intersection turn, 71 frames): **per-step yaw correlation 0.09-0.23 (3D > 2D 0.10 — elevation helps but is swamped), rot p90 ~170 deg Kabsch flips, cumulative heading garbage across all tau_d / baseline N=2,4,8 / gated configs; TRANSLATION roughly tracks (~1.3 m vs 0.95 m GT step).** Same verdict as the real 2D ARS408 — the synth-4D 0.995 was an artifact of its IDEALIZED scatterer persistence + continuous elevation + ~200 inliers; real radar gives 1-deg-quantized elevation + ~15-30 flickering inliers -> noise floor ~10 deg buries the ~1 deg/frame signal. Binding constraint = persistence + elevation resolution, NOT raw detection count. **Promising real-radar rotation direction (noted, not built): a Doppler-native (v, omega) per-frame estimator** — the Doppler ego-velocity fit is confounded by the turn's omega x r term, i.e. the yaw-rate signal lives in the Doppler, not the scan-match geometry. Calibrator-feed (ofc_replay) of the radar rotation extrinsic is therefore MOOT (a front-end with no usable rotation can't pin one); radar-as-translation-source would mirror the already-documented 2D case.
...
- **REAL 4D RADAR — RADIal: DONE, negative (the synth-4D rotation unlock does NOT transfer).** Full write-up `RADAR_SCAN_ODOMETRY.md` §5 + §3 SESSION DELTA. Tools: `tools/radial_to_csv.py` (radar 3D scan-match + lidar-GT validation), `tools/proto_radial_lidar_odom.py` (lidar ICP reference), data-prep multipart-zip reader/extractor in `C:\workspace\data\RaDIaL\_recon\`. Real RADIal radar reproduces the 2D rotation wall (yaw corr ~0.1-0.23, translation usable) — persistence + 1-deg-quantized elevation, not detection count, are the binding constraints. Open follow-on if revisited: a **Doppler-native (v, omega) rotation estimator** (the yaw-rate signal is in the Doppler, not the scan-match geometry); a denser/finer-elevation imaging radar with stable returns.
