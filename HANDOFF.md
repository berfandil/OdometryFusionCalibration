...
- ~~**GPS R-side honest NEES**~~ **DONE (`28339aa`, `GPS_R_NEES_SWEEP.md`)**: `cov_floor_m2=25` is 17–300× overconfident; raised the recipe to **~300** (config-only win — better accuracy + consistency + urban12 worst-window). Full NEES~6 is structural (honest-R 7× cross-drive; confounded by GPS-vs-GT frame disagreement ~8–26 m; flat per-fix variance) → a per-fix/multipath-aware R MODEL + frame-offset characterization is the principled follow-up (ISSUES).
