...
- [x] **REAL 4D radar (RADIal, `tools/radial_to_csv.py` + `tools/proto_radial_lidar_odom.py`): rotation does NOT unlock — see §5.** The synth unlock did NOT transfer: real RADIal radar (~500 det/frame, but 1-deg-quantized elevation + poor scatterer persistence → ~15-30 inliers vs synth ~200) reproduces the 2D wall — per-step yaw correlation 0.09-0.23 (3D > 2D 0.10, so elevation helps but is swamped), rot p90 ~170 deg Kabsch flips, cumulative heading garbage; TRANSLATION roughly tracks (~1.3 m vs 0.95 m GT step). Lidar ICP odometry is the reference/GT (no GPS/CAN in RADIal Ready_to_use). The synth-4D 0.995 was an artifact of idealized persistence + continuous elevation. Promising real-radar rotation direction = a Doppler-native (v, omega) estimator, NOT this scan-match (§5).
...
**Static filter:** could NOT be reliably calibrated on RADIal — the radar-Doppler-implied ego speed (~6-8 m/s from the per-frame `v_radial = a cos(az) + b sin(az)` fit, 80-100% inlier) disagrees ~2x with the lidar-ICP-implied speed (~4 m/s), and the exact frame rate is ambiguous; both Doppler signs reject ~95% of detections at any tried hz. Moot for the rotation verdict (geometry test ran static-off). **NOTE for a future radar-rotation attempt:** that the Doppler ego-velocity fit is CONFOUNDED by the turn (the omega x r term it cannot absorb into a planar (vx,vy)) is itself evidence the **yaw-rate signal lives in the DOPPLER field**, not the scan-match geometry — a Doppler-native rotation estimator (fit full (v, omega) per frame from the azimuth-dependent Doppler) is the promising direction real imaging radar actually supports, distinct from this descriptor scan-match.

**Conclusion:** the descriptor scan-match recovers usable radar TRANSLATION on real 4D radar but NOT rotation; the synth-4D unlock was an artifact of idealized persistence + continuous elevation. The binding constraint on real imaging radar is **scatterer persistence + elevation resolution**, not raw detection count — so density alone (the synth's swept variable) is necessary but NOT sufficient. The Slice-20b radar-lever unblock therefore does not land on RADIal-class 4D radar via this front-end; rotation needs either a Doppler-native estimator (above) or a denser/finer-elevation imaging radar with stable returns. Tools: `tools/radial_to_csv.py` (radar 3D scan-match + lidar-GT validation), `tools/proto_radial_lidar_odom.py` (lidar ICP reference), `tools/synth_4d_radar.py` (the synth contrast). Data prep (the RADIal split-zip is misnamed parts with no 7z on the box): a Python multipart-zip reader + run-extractor under `C:\workspace\data\RaDIaL\_recon\`.
