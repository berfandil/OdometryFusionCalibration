...
- [x] **4D/dense radar (`tools/synth_4d_radar.py`, `345c782`): rotation UNLOCKED on SYNTH** — yaw correlation 0→0.995 with density, cumulative heading tracks GT to ~1 deg, fused drift 801 m→1.7 m / NEES 78→0.3 / extr_rot conf 0→0.45 / scale→1.0 (see §4 lever (3) for the full table). Density/3D removes the rotation wall via this algorithm; the 2D limit was sensor-bound. The 3D path (`so3_log`/`static_filter_3d`/3D Kabsch) is in `radar_scan_odometry.py` + `synth_4d_radar.py`. Latent 2D yaw-sign-flip bug fixed (`e0285f1`).
- [x] **REAL 4D radar (RADIal, `tools/radial_to_csv.py` + `tools/proto_radial_lidar_odom.py`): rotation does NOT unlock — see §5.** The synth unlock did NOT transfer: real RADIal radar (~500 det/frame, but 1-deg-quantized elevation + poor scatterer persistence → ~15-30 inliers vs synth ~200) reproduces the 2D wall — per-step yaw correlation 0.09-0.23 (3D > 2D 0.10, so elevation helps but is swamped), rot p90 ~170 deg Kabsch flips, cumulative heading garbage; TRANSLATION roughly tracks (~1.3 m vs 0.95 m GT step). Lidar ICP odometry is the reference/GT (no GPS/CAN in RADIal Ready_to_use). The synth-4D 0.995 was an artifact of idealized persistence + continuous elevation. The Doppler-native (v, omega) alternative was ALSO prototyped + falsified (§6): omega couples to a single radar's Doppler only via the lever-arm lateral velocity, buried under forward-FOV noise. BOTH rotation paths sensor-bound on RADIal's single front radar; usable channel = Doppler TRANSLATION only.
...
## 5. REAL 4D imaging radar (RADIal) — the synth-4D rotation unlock does NOT transfer (2026-06-19)

The §4 (3) unlock was proven on a SYNTHETIC 4D radar (a fixed landmark cloud + continuous elevation + the real ego trajectory). The open question it left: does the same scan-match recover rotation on a REAL imaging radar? Tested on **RADIal** (Valeo, TI-cascade 76 GHz 4D radar — 3D position + Doppler; data at `C:\workspace\data\RaDIaL`). **Verdict: NO. Real 4D radar reproduces the real-2D-ARS408 rotation wall — translation roughly tracks, per-step rotation stays below the Kabsch noise floor — despite ~500 detections/frame and an elevation channel.** The synth-4D 0.995 was driven by its IDEALIZATIONS (perfect scatterer persistence + continuous elevation + 200 inliers), which real radar violates.

**Setup (`tools/radial_to_csv.py` + `tools/proto_radial_lidar_odom.py`):** RADIal Ready_to_use ships NO GPS/CAN/ego-pose (only camera, the Valeo Scala lidar, the radar, freespace labels) — so the dense lidar (laser_PCL, ~14k pts) is the reference + GT: `proto_radial_lidar_odom.py` recovers ego SE(3) by **coarse-to-fine point-to-point ICP** (voxel-downsample ~1200 pts; rms ~0.27 m = the Scala point-spacing floor; identity init each frame avoids cascade). The radar runs the PROVEN 3D scan-match (RADAR_SCAN_ODOMETRY 3D path: `static_filter_3d` + 3D descriptors + 3D Kabsch/RANSAC); the lidar ego delta is the static-filter prior (replaces nuScenes CAN) AND the GT to score the radar yaw against. Frames carry NO timestamp; the inter-frame rate is **~8 Hz** (from the radar-Doppler-ego-speed vs lidar-displacement match in §6 — an earlier turn-confounded non-RANSAC estimate gave ~4; native radar is likely faster with the released subset sampled). **The rotation test is dt-INDEPENDENT (Kabsch pose delta) and gate-INDEPENDENT (run with the plausibility gate off), so the exact hz does not affect the verdict.**

**Best run: `RECORD@2020-11-22_12.53.55` frames 113-184 — an 81 deg intersection turn, 54 m, 71 frames** (picked by surveying 25 contiguous runs for net yaw; RADIal's released frames are a SAMPLED subset, so contiguous-index runs are short — longest 240 frames, most are slow stop-go). Lidar GT: net yaw -76 deg, per-step yaw **std 1.17 deg, range [-3.9, +0.3]** (~1.1 deg/frame).

**Real radar facts vs the synth:** ~500 detections/frame (above the synth N=500 that hit 0.995) BUT (a) elevation is **1-deg QUANTIZED** (-4..6 deg integer bins) -> near-planar vertical conditioning, vs the synth's continuous elevation; (b) frame-to-frame scatterer **persistence is poor** (RCS fluctuation / multipath / occlusion) -> only **~15-30 RANSAC inliers**, vs the synth's ~200.

**Result (the rotation HEADLINE — per-step yaw correlation recovered-vs-GT):**

| config (gate off, static off, 300-400 det) | yaw corr | inliers | rot med | rot p90 | cum heading (GT -76 deg) |
|---|---|---|---|---|---|
| N=1 td=1.0 | 0.09 | 16 | 9.9 deg | 138 deg | -579 deg |
| N=1 td=2.0 | 0.41* | 11 | 10.0 deg | 169 deg | -1143 deg |
| N=2 / N=4 / N=8 (longer baseline) | -0.07 / 0.13 / 0.12 | 11 / 11 / 9 | 20 deg | ~170 deg | garbage |
| 3D vs 2D (z-zeroed), per-pair | **0.23** vs 0.10 | — | — | — | — |

(*the 0.41 is spurious — its cumulative heading is -1143 deg; a few aligned frames amid blunders inflate the Pearson. Every config's cumulative heading is garbage.) **rot_p90 ~140-170 deg = catastrophic Kabsch 180-deg flips** (near-degenerate planar geometry from the quantized elevation). The plausibility gate at the correct hz rejects the flips (42% gated) but the SURVIVORS still give corr **-0.31** and a +49 deg cumulative (wrong even in sign). **3D beats 2D (0.23 > 0.10) — elevation DOES help — but both are swamped:** the per-step rotation noise floor (~10 deg, blunders to 170 deg) buries the ~1 deg per-step signal. SNR ~ 1/100 of the synth.

**Why baseline N>1 fails (same as the real-2D falsification, §4 (1)):** accumulating yaw over N frames raises the signal ~N deg, but the larger radar displacement collapses FOV overlap -> inliers drop 16->9 -> the noise floor rises as fast. No sweet-spot.

**Translation:** magnitude right but noisy on THIS turn run — median per-step translation error ~1.3 m vs the ~0.95 m GT step (the sharp turn churns the FOV, degrading correspondences for translation too, though it never produces the catastrophic 180-deg blunders rotation does). Directionally the same as the 2D verdict (**translation the usable channel, rotation not**) but the per-step translation error here is comparable to the step — not the clean ~0.5 m the straighter 2D nuScenes runs gave. A straighter RADIal run would translate more cleanly; rotation fails on ANY run (the noise floor is sensor-bound, not motion-dependent).

**Static filter:** moot for the rotation verdict (the geometry test ran static-off). The earlier apparent radar-Doppler-speed (~7.8 m/s) vs lidar-speed (~3.9 m/s) ~2x DISAGREEMENT that made the static filter reject ~95% of detections was simply the **hz error** — at the corrected ~8 Hz the lidar speed (disp x 8 ~ 7.6 m/s) MATCHES the radar Doppler speed (§6), so the Doppler ego-velocity is consistent; the static filter would calibrate at hz ~8. Not re-run (irrelevant to the static-off geometry verdict).

**Conclusion:** the descriptor scan-match recovers usable radar TRANSLATION on real 4D radar but NOT rotation; the synth-4D unlock was an artifact of idealized persistence + continuous elevation. The binding constraint on real imaging radar is **scatterer persistence + elevation resolution**, not raw detection count — so density alone (the synth's swept variable) is necessary but NOT sufficient. The Slice-20b radar-lever unblock therefore does not land on RADIal-class 4D radar via this front-end. **The Doppler-native (v, omega) rotation estimator — the §5 forward direction — was prototyped and ALSO fails on RADIal's single forward radar (§6): omega couples to a single radar's Doppler ONLY through the lever-arm lateral velocity, which is buried under the forward-FOV lateral-velocity noise.** Both rotation paths are sensor-bound on RADIal-class hardware; the radar's one usable channel is Doppler ego-velocity (TRANSLATION). Tools: `tools/radial_to_csv.py` (radar 3D scan-match + lidar-GT validation), `tools/proto_radial_lidar_odom.py` (lidar ICP reference), `tools/proto_radial_doppler_yaw.py` (the Doppler-native test, §6), `tools/synth_4d_radar.py` (the synth contrast). Data prep (the RADIal split-zip is misnamed parts with no 7z on the box): a Python multipart-zip reader + run-extractor under `C:\workspace\data\RaDIaL\_recon\`.

---

## 6. Doppler-native (v, omega) rotation — prototyped, ALSO fails (single forward radar) (2026-06-19)

The §5 forward direction: since per-point scan-match geometry buries rotation, try the DOPPLER. `tools/proto_radial_doppler_yaw.py` tests it on the same 81-deg-turn run, lidar ICP as GT.

**Observability (the design + its limit).** A static world point at sensor position r (bearing u=r/|r|) shows radial velocity `v_radial = -(v + omega x r).u = -v.u` — because `(omega x r).u = 0` (omega x r is perpendicular to r). **So a single radar's instantaneous Doppler gives ONLY the linear velocity v; omega does NOT appear per-point.** omega couples in ONLY through the lever arm: the radar's own linear velocity is `v_radar = v_cog + omega x L`; for a forward lever Lx under near-non-holonomic motion the LATERAL component is `v_y = omega_z * Lx`, so `omega_z = v_y / Lx` (the same lever<->rotation coupling the calibrator uses, run forward). Method: per-frame RANSAC fit `v_radial = a cos(az) + b sin(az)` (a=-vx, b=-vy), mover-rejecting; the HEADLINE is whether the lateral velocity `v_y` correlates with the lidar yaw rate.

**Result:**
- **LINEAR velocity WORKS** — radar forward speed |vx| med **7.78 m/s** vs lidar **7.64 m/s** at hz 8 (magnitude match; this also CALIBRATES the rate to ~8 Hz). So the radar IS a usable Doppler ego-VELOCITY (translation) source — exactly the nuScenes `translation_only` front-end.
- **YAW is BURIED** — `corr(v_y, lidar per-frame yaw) = -0.147` (~0). `v_y` std **2.3 m/s** swamps the lever signal (`omega*Lx ~ 0.3-1 m/s` typical, ~1.9 m/s only at the turn peak). The lever fit (`v_y = omega_z*Lx`) returns a meaningless Lx (fitting noise); integrated yaw -260 deg vs GT -76 deg.

**Why (a geometric limit, not tuning):** two compounding facts. (1) The yaw signal reaches Doppler only as the small lever-arm lateral velocity `omega*Lx` (~sub-m/s at urban yaw rates). (2) A forward-looking radar with a **+-53 deg azimuth FOV** weakly constrains the lateral velocity itself — the `sin(az)` basis column is small when detections cluster near boresight — so the fitted `v_y` carries ~2.3 m/s noise. Signal << noise. (A side/corner radar, or a multi-radar rig like nuScenes' 5, would constrain `v_y` far better — but RADIal has a SINGLE front radar.)

**Net:** on RADIal's single forward imaging radar, BOTH rotation paths fail — descriptor scan-match (§5: persistence + 1-deg elevation) and Doppler-native (§6: lever signal below forward-FOV lateral-velocity noise). The radar contributes a usable Doppler TRANSLATION only, matching the original nuScenes verdict. Recovering rotation from radar needs a sensor with stable dense returns + finer elevation (scan-match) or strong lateral-velocity observability — wide/side FOV or a multi-radar rig (Doppler-native).
