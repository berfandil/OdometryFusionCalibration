...
- (3) **A 4D/imaging radar (§2) — now the leading fix**: more detections + elevation → a far better-conditioned 3D Kabsch and a slower-collapsing overlap. The 2D-ARS408 rotation wall is fundamentally point-count/conditioning-limited, which a denser sensor addresses directly.

Translation is already usable as a drop-in radar source today. Committing a real radar LEVER remains gated on this front-end rotation quality (Slice 20b) — and on 2D ARS408 that quality is sensor-bound, not algorithm-bound.
