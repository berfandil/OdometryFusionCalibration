...
- **Impl note (Slice 2)**: error-state ordering is `[trans; rot]` (pose 0–5, twist 6–11). Predict on a delta uses right-error transport `F = blkdiag(Ad(delta⁻¹), 0)` (twist re-read each step as `log(delta)/dt`, so its prior error doesn't carry), `P ← F P Fᵀ + blkdiag(Q, Q/dt²)`. Adaptive `Q = q_scale·spread² · I₆ + diag(q_floor)`. Frame-align of a source-frame delta `B` to base: `A = X ∘ B ∘ X⁻¹` with `X = prior_extrinsic` (sensor→base). Accepted Slice-2 approximations: zero pose↔twist cross-covariance; `sigma_confidence` unit-mixing (until Slice 9 reliability EMA).
