...
- **Impl note (Slice 14, approach A — median-variance reduction, `730bcfa`)**: the spread-derived Q was ~14–17× too LARGE because `spread` measures inter-source DISAGREEMENT (≈ a single source's dispersion), NOT the fused estimate's accuracy — and the geometric median of `n` agreeing sources has ~`1/n` the single-source per-window variance. So the adaptive Q now divides the spread term by the participating-median source count `n_eff` (`q_pose = q_scale·spread²/max(1,n_eff)·I + q_floor`; floor undivided; `adaptive_q_source_reduction` knob, default on, recovers the old behavior when off). This is the principled, parameter-free, **safe** part of the no-ref-pessimism fix (predict-only NEES 0.35 → ~1.0, still conservative). The residual ~6× was the un-calibrated `q_scale=1` coefficient, **now calibrated** (Slice-14 sweep, `983fa65`): `q_scale 1.0 → 0.5`. Selection rule = the max pessimism cut SUBJECT TO the filter never being overconfident (ensemble-mean NEES ≤ DOF) on ANY trajectory in {nees_traj, mixed, turning, straight} × {1×,2× noise}, with a conservative margin (worst-case NEES ~2.9–3.5, not 6). **Rejected**: a magic constant tuned to force NEES≈6 on one trajectory (≈`q_scale 0.17`) — it overfits and pushes other trajectories overconfident (≥7 on `turning` at `q_scale 0.2`), and the sim under-states real model mismatch, so mild pessimism is preferred over any overconfidence for this safety component. A distance-aware *shape* model / NHC no-ref pseudo-correction remain the path to strict per-trajectory consistency but are deferred (the spike found Q-over-conservatism, not the transport shape, is the dominant lever). `q_floor` default kept small (the GPS-gain trade is a documented deployment choice, not a single best value — CONFIG §3).
