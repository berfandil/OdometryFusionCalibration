...
- **Impl note (Slice 14, covariance tangent)**: the "decoupled `SO(3)×ℝ³`" wording describes the error *ordering* (`[trans; rot]`) and the **median's** split metric — NOT the covariance. The ESKF propagates `P` with the **full SE(3) adjoint** `F = Ad(delta⁻¹)` (the `[t]ₓR` coupling block), so the dense 12×12 covariance lives in the **full coupled SE(3) tangent**; NEES/consistency must use the full-SE(3) `se3::log` error to match it (verified in Slice-14 review). The covariance is currently **mis-calibrated (pessimistic ~46×)** — see DESIGN §10 / the Slice-14 finding.
