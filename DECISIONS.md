...
- **Impl note (Slice 11, correction path)**: the **plugin + Mahalanobis-gated ESKF update half** of D22 shipped (`Eskf::update`: `S=HPHᵀ+R`, NIS gate, gain via one LDLT, Joseph covariance, right-error full-SE(3) injection; `Estimator::add_correction` wired into `step()` after predict; `Result::CorrectionDiag`; `validate()` `mahalanobis_chi2>0`). Sim `SyntheticAbsoluteRef` position fix → drift removed (0.58→0.20 m); gross outlier rejected (NIS ~3e5); NIS now computable (~2.4 vs DOF 3). The gate is a **single scalar regardless of measurement DOF** (fine for the dim=3 fixes; needs a per-`n` χ² quantile for 6-DOF/mixed plugins). **Bias states + GPS adapter DEFERRED to Slice 11b** — the core 12-DOF state is NOT augmented; `bias_states` is a no-op.
- **Open decision (Slice 11b, deferred 2026-06-07)**: D22's GPS/INS bias-estimation assumes the biased sensor *drives the predict*, but here the **median of N sources** drives it — so the pose↔bias cross-covariance that makes a bias observable doesn't form classically. Two candidate designs recorded in `ISSUES.md` → Slice 11b DESIGN NOTE: **(A)** an augmented `[pose;twist;bias]` filter for the *single-driving-source* regime (low risk, exact GPS/INS structure), and **(B)** full median-coupled bias via the per-source median weight in the augmented predict `F` (general N-source case, higher risk). To be decided when revisiting after the rest is done (recommendation: A first). Also pending in 11b: the per-`n` χ² Mahalanobis gate.
